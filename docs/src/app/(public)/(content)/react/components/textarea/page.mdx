...
import { DemoTextareaHero } from './demos/hero';
...
import { DemoInfiniteResize } from './demos/infinite-resize';
...
import { DemoMaxRows } from './demos/max-rows';
...
import { DemoTextareaForm } from './demos/form';
...
The Textarea component automatically detects browser support for `field-sizing: content` at runtime using `CSS.supports()`. When supported, it uses native CSS field sizing for optimal performance. When unsupported, it falls back to JavaScript-based resizing logic. This ensures the component works everywhere while taking advantage of modern browser capabilities.
...
import { DemoFieldSizing } from './demos/field-sizing';
